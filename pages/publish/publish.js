...
Page({
  ...
  //构建categoryObject{}
  setCategoryObject:function(categoryArr){
    let CategoryObject=[]
    categoryArr.forEach((item,index)=>{
      ...
      this.setData({
        CategoryObject:this.data.CategoryObject.concat(CategoryObject)
      })
    })
  },
  formSubmit: function(e) {
    ...
    if(!userId){
      ...
      if (e.detail.value.category && e.detail.value.price&&e.detail.value.nowPrice && e.detail.value.title) {
        ...
        wx.request({
          url: app.globalData.apiConfig.publish_url,
          data:{
            "userId":userId,
            "goodsName":e.detail.value.title,
            "category":that.data.category+1,
            "goodsPrice":e.detail.value.nowPrice,
            "originalPrice":e.detail.value.price,
            "quality":e.detail.value.qua? e.detail.value.qua:"",
            "goodsDesc":e.detail.value.desc? e.detail.value.desc:"",
            "goodsImg":that.data.uploadImg.toString()
          },
          header: {
...
//上传图片到cos
  ...
  onLoad: function (options) {
      let that=this
      let cateGoryArr=[].concat(wx.getStorageSync('category'))
      cateGoryArr.forEach((item,index)=>{
          that.setData({
            array:that.data.array.concat(item.categoryname)
          })
      })
